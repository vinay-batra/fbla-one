...
**LIVE in production at [fbla.one](https://fbla.one).** Last shipped: v1.3 (June 3, 2026) - public AI chat (on every page, logo button, animated thinking dots, short replies, no auto-scroll on output) + report-bug flag button + hero redesign + auth/sign-in fix + NEW blue arrow/book logo everywhere + a 6-agent audit pass. **ACTION REQUIRED: apply migration 0009** (`supabase/migrations/0009_feedback_user_id_guard.sql`) in the Supabase SQL editor - binds `feedback.user_id` to the caller. Migrations 0001-0008 applied + verified.
