...
- **STILL OPEN (deliberately deferred, NOT done):** **#47** - split the ~1218-line `app/app/chapter/page.tsx` into setup/advisor/member/deadlines (pure structure; do it with live regression of the auth-gated advisor + member views). **#20** - cross-instance rate limiting (needs an Upstash/Vercel KV account). Resend email + VAPID push remain scaffolded/blocked on external accounts. `email_signups` has no admin UI yet (read it via SQL editor).
