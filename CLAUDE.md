...
**LIVE at [fbla.one](https://fbla.one). Last shipped: v1.6.2 (June 8, 2026) - audit remediation (all 59 findings fixed).** **Migrations: 0001-0017 in repo** (0001-0015 applied + verified live; **0016 + 0017 are idempotent - apply both in the SQL editor if not already**). Read this v1.6 block first; the v1.5/v1.4/v1.3 blocks below are still accurate history.

> **v1.6.2 - audit remediation (June 8, 2026).** Fixed every finding from the v1.6 audit (0 Crit, 0 High, 19 Med, 35 Low, 5 Info). tsc/lint/build clean, `npm audit` 0 vulns, full live regression (advisor + member chapter flows, auth, tracker, email CTA, coach) passed with no console errors. See CHANGELOG v1.6.2 for the per-finding list. **ACTION REQUIRED:** (1) apply `supabase/migrations/0017_audit_remediation.sql` in the SQL editor (indexes, role/feedback/email-list guards, audit_log, email_signups service-role delete grant, 8-char invite codes) - the app tolerates it being unapplied (best-effort erasure/audit, EmailCta RPC falls back to a direct insert). (2) Optional: add a `SUPABASE_DB_URL` GitHub secret to enable the nightly backup workflow (`.github/workflows/db-backup.yml`), and set `NEXT_PUBLIC_TURNSTILE_SITE_KEY` + Supabase captcha. (3) The CSP in `next.config.ts` ships **report-only**; flip to enforce after validating against the live page. New shared modules: `lib/url.ts` (safeNextPath), `lib/leaderboard-cache.ts`, `lib/chapter.ts` `AI_LOG_PREFIX`. One stray test row (`audit2-list-...@example.com`) is in `email_signups` from live verification - delete via SQL editor (or after 0017 grants delete).
