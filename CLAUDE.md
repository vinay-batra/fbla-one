...
**LIVE at [fbla.one](https://fbla.one). Last shipped: v1.6.2 (June 8, 2026) - audit remediation (all 59 findings fixed).** **Migrations: 0001-0017, ALL applied + verified live** (RLS 18/18 via `_rls_test.mjs`, leaderboard 8/8 via `_leaderboard_test.mjs`, 0017 behaviors verified live: 8-char invite codes, officer self-assign blocked, email_signups service-role delete works, join_email_list RPC + anon-insert revoke). Read this v1.6 block first; the v1.5/v1.4/v1.3 blocks below are still accurate history.
